...
def train_step(model, data_loader, optimizer, scheduler, device, train_losses):
    model.train()
    loss_sum = 0
    ...
    epoch_loss = loss_sum/n             # Dumebi U Insertion 
    train_losses.append(epoch_loss)     # Dumebi U Insertion 
    stats = {'train_loss': loss_sum / n}
    if scheduler:
        scheduler.step(loss_sum)
    return stats, train_losses
