import numpy as np
import torch_geometric.nn as gnn  ## dumebi inclusion
import math
...
#Dumebi Insertion: To Collect Losses 
class HistoryCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        self.epoch_losses.append(logs.get('loss'))  # Record only training loss
...
class DNN(keras.Model):
    ...
    def fit(self, x_train, y_train, **kwargs):
        ...
        history_callback = HistoryCallback()  # Initiating the Loss Callbacks 

        super().fit(x_train, y_train, epochs=self.epochs, batch_size=self.batch_size, callbacks=[history_callback],**kwargs)

        #Dumebi Insertion-->Storing the calculated losses 
        history_df = pd.DataFrame({
            'epoch': range(1, self.epochs + 1),
            'loss': history_callback.epoch_losses
        })
    ...
    def save_weights(self, fn):
        ...
        if fn.endswith('.h5'):
            # Save in HDF5 format (inferred from the extension)
            super().save_weights(fn)
        elif fn.endswith('.chkpt'):
            # Save in TensorFlow checkpoint format (inferred from the extension)
            super().save_weights(fn)
        else:
